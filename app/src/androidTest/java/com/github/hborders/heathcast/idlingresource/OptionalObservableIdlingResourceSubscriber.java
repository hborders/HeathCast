...
public final class OptionalObservableIdlingResourceSubscriber {
    public static SubscribedIdlingResource subscribe(
            String name,
            Observable<? extends Optional<?>> optionalObservable
    ) {
        ...
        final AtomicReference<IdlingResource.ResourceCallback> resourceCallbackAtomicReference =
                new AtomicReference<>();
        final Disposable disposable = optionalObservable.subscribe(optional -> {
            final boolean isIdle = !optional.isPresent();
            isIdleAtomicBoolean.set(isIdle);
            if (isIdle) {
    ...
    public static class SubscribedIdlingResource implements IdlingResource, Disposable {
        ...
        private final AtomicReference<IdlingResource.ResourceCallback> resourceCallbackAtomicReference;
